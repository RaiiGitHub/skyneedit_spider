...
class DbOperatorTYC extends dbop {
    ...
    ensureTablesExist(callback) {
        ...
        var enterprise_base = "CREATE TABLE IF NOT EXISTS `enterprise_base`(\
                        ...
                        `shortName` VARCHAR(255) NULL COMMENT '简称（源于搜索关键字）', \
        ...
        var search_tasks = "CREATE TABLE IF NOT EXISTS `search_tasks`(\
                        `id` int auto_increment primary key COMMENT '自增长的键值',\
                        `from` int NOT NULL COMMENT '搜索关键字的起始索引',\
                        `to` int NOT NULL COMMENT '搜索关键字的结束索引',\
                        `status` VARCHAR(45) NULL COMMENT '运行状态-running,idle',\
                        `pages` timestamp NULL DEFAULT '0000-00-00 00:00:00' COMMENT '搜索开始时间',\
                        `searchEndtTime` timestamp NULL DEFAULT '0000-00-00 00:00:00' COMMENT '搜索结束时间');";
    ...
    insertSeartKeys(filepath, matchobj, callback) {
                ...
                self.verifySearchKeyExists(km.key, function (ok) {
                    if (!ok) {
                        ...
                        self.connection_.query(q, function (error, results, fields) {
                            if (error) {
                                log._logE('Mysql::verifySearchKeyExists', 'exist,or unknown...', q);
                                callback(false);
                            }
                        });
                        console.log(q);
    ...
    insertCompany(desc, callback) {
        ...
        self.verifyCompanyExists(desc.company_id, function (exists) {
            if (exists) {
                ...
                var q = printf("insert into enterprise_base(code,shortName,fullName,url,briefDesc,recordTime) values('%s','%s','%s','%s','%s',NOW());",
